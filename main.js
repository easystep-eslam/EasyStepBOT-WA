...
// ===== React Raffle (reactions draw) =====
let reactraffle = null;
try { reactraffle = require('./commands/Tools/reactraffle'); } catch { try { reactraffle = require('./commands/tools/reactraffle'); } catch {} }

function _rrNormalizeJid(jid = '') {
  return String(jid).split(':')[0];
}

function _rrTryStoreReaction(msg) {
  try {
    ...
    if (!targetId || !reactraffle?.readDB || !reactraffle?.writeDB || !reactor) return;

    const db = reactraffle.readDB();
    const raffle = db?.[chatId]?.[targetId];
    if (!raffle || !raffle.active || !Array.isArray(raffle.entries)) return;

    if (!raffle.entries.includes(reactor)) {
      raffle.entries.push(reactor);
      reactraffle.writeDB(db);
    }
  } catch {}
}
// =========================================
