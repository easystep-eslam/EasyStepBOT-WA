...
async function getAutoresbot(url) {
  ...
  console.log('[AUTORESBOT STATUS]', r.status)
  ...
  console.log('[AUTORESBOT DATA]', r.data)

  if (!r?.data?.status) throw new Error('AUTORESBOT_FAILED')

  const dl = r?.data?.data?.url

  const title = r?.data?.data?.title || 'audio'

  const thumb = r?.data?.data?.thumbnail || null

  if (!dl) throw new Error('AUTORESBOT_NO_URL')
