...
function resolveLocaleFromChat(chatId) {
    const lang = chatId ? getLang(chatId) : 'en'
    return lang === 'ar' ? { lang: 'ar', locale: 'ar-EG' } : { lang: 'en', locale: 'en-US' }
}
...
/**
 * ✅ getTime — توقيت مصر + لغة الجروب
 * @param {string} format
 * @param {Date|number|string} date
 * @param {string|null} chatId
 */
exports.getTime = (format, date, chatId = null) => {
    const { lang } = resolveLocaleFromChat(chatId)
    const tz = DEFAULT_TZ

    if (date) {
        return moment(date).tz(tz).locale(lang).format(format)
    } else {
...
/**
 * ✅ formatDate — تواريخ وأيام/شهور حسب لغة الجروب + توقيت مصر
 * @param {number|string|Date} n
 * @param {string|null} chatId
 */
exports.formatDate = (n, chatId = null) => {
    const { locale } = resolveLocaleFromChat(chatId)
...
/**
 * ✅ tanggal — أيام/شهور حسب لغة الجروب + توقيت مصر
 * (بنفس اسم الدالة عشان ما نكسرش أي استدعاء قديم)
 * @param {number|string|Date} numer
 * @param {string|null} chatId
 */
exports.tanggal = (numer, chatId = null) => {
    const { locale } = resolveLocaleFromChat(chatId)
...
/**
 * ✅ jam — إصلاح الباج + توقيت مصر افتراضيًا
 * options: { format?: "HH:mm", timeZone?: "Africa/Cairo" }
 */
exports.jam = (numer, options = {}) => {
    let format = options.format ? options.format : "HH:mm"
    const tz = options.timeZone ? options.timeZone : DEFAULT_TZ
    let jam = moment(numer).tz(tz).format(format)
    return `${jam}`
}
